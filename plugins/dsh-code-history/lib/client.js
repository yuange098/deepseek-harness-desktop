window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function extract(trajectory, chatSources) {
			...
			for (const item of answers) unique.set(`${item.lang}|${item.code}`, item);
...
`;
		...
		function CodeView3(props) {
			...
			const count = tab === "executed" ? flat.length : answers.length;
			...
			return h("div", { className: "ch-root" },
				...
				count === 0
					? h("div", { className: "ch-empty" }, "这个会话还没有代码记录",
						h("div", { className: "ch-sub" }, "轨迹 " + trajKey + " 条｜消息 " + chatKey + " 条"))
					: tab === "answers"
						? h("div", { className: "ch-grid" }, answers.map((item, i) => h(Tile, { key: "a" + i, item })))
						: layout === "flat"
