...
### 3. 插件 `dsh-code-history` —— 会话里的「**代码和文本**」页签

在对话页签后面多出一个「代码和文本」页签，里面分**两个互不混合的模块**：
...
| **只留完整可运行的** | 按"有没有入口/主流程、有没有读写数据或绘图、有多长"判完整性；只有几行的片段（报错示例、单行改法）默认**不展示**，点「含片段」才出来 |
| **AI 判定（默认开）** | 每段代码再交给模型判一次："能不能独立完成一个功能"（画图/合并数据/批量重命名…）。判为片段 → 自动折叠；判为可用 → 用 **AI 起的中文功能名**当标题。结果按代码指纹缓存在 `home/cache/code-history/judge.json`，**同一段只问一次**，翻历史不再花钱；可用工具栏「AI 判断」关掉 |
| **一个任务一张主卡** | 同一任务里的一堆过程脚本不再平铺：只摊开最完整的那份，其余收进「过程文件（N）」里，需要时再展开 |
| **任务意图过滤** | 写文章、润色、翻译、整理文档这类文字任务里产生的代码默认**折叠**，点「含文字任务」可随时调出来；提问抓不到时一律按代码任务保留（宁可多留，不误伤） |
...
| 能力 | 说明 |
| --- | --- |
| 只收成品 | 写得够长的回答正文、以及**写成文件的文字**（.md/.txt/.tex…）才会进来；"我先看看…"这类过程话不收 |
| 同段只留最后一版 | 同一段被反复润色/改写时，只保留**最后一次**的内容，卡片上标「共 N 版 · 只留最新」；同一文件写多遍也只留最后一次 |
| 文件也进来，但与代码分开 | 生成的文档/数据文件（.md/.csv/.xlsx…）登记在文本模块里、带「文件」徽标，代码模块里不会出现它们 |
| 阅读与复制 | 标题 + 字数 + 段数，超长默认折叠、可展开全文，一键复制或导出 Markdown |
